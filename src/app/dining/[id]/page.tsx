...
export default async function DiningDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ plan?: string }> }) {
  const me = await currentUser();
  if (!me) redirect("/switch");
  const { id } = await params;
  const { plan } = await searchParams;
  const item = getItem("dining", id);
  if (!item) notFound();
  return (
    <>
